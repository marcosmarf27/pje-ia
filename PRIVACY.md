...
| **Perfil Google** (nome, e-mail e foto), *apenas se você clicar em “Entrar com Google”* | Identificar no popup/opções com qual conta você está logado — é **opcional** e **não bloqueia** nenhuma funcionalidade | O login usa o `chrome.identity` do navegador e fala **direto com o Google** (`accounts.google.com`), sem servidor intermediário. Nome, e-mail e URL da foto são lidos do token de identidade e guardados **somente** no `chrome.storage.local` deste computador (não sincronizam, não vão ao provedor de IA nem ao desenvolvedor). O botão **Sair** apaga esses dados na hora. |
